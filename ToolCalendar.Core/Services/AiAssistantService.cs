...
Lưu ý: Không dùng JSON. Chỉ trả lời bằng Markdown bình thường và thêm tag đặc biệt ở cuối nếu cần.";
            ...
            for (int hop = 0; hop <= MaxToolCalls; hop++)
            {
                ...
                var requestBody = isLastHop
                    ? (object)new { model = _modelName, messages = messages, stream = false, keep_alive = -1, options = new { temperature = 0.0 } }
                    : (object)new { model = _modelName, messages = messages, stream = false, tools = tools, keep_alive = -1, options = new { temperature = 0.0 } };
