...
- **Tệp thay đổi**:
  - `ToolCalendar.Api/**/*.cs` (Sửa đổi)
  - `ToolCalendar.Core/**/*.cs` (Sửa đổi)
- **Lệnh git commit**: `git commit -m "style: format code chuẩn C#"`
...
- **Lệnh git commit**: `git commit -m "fix(style): chống đè và cắt xén giao diện trang Cấu hình khi zoom lớn"`
...
- **Tệp thay đổi**:
  - `ToolCalendar.Api/ClientApp/src/features/documents/routes/UploadPage/index.jsx` (Sửa đổi)
- **Lệnh git commit**: `git commit -m "fix(style): bỏ min-h-0 panel trái UploadPage để hiện đủ Chờ rà soát"`
...
- **Tệp thay đổi**:
  - `ToolCalendar.Core/Services/Security/HmacRequestHandler.cs` (Sửa đổi)
- **Lệnh git commit**: `git commit -m "fix(infra): add missing using Microsoft.AspNetCore.Http in HmacRequestHandler"`
...
- **Tệp thay đổi**:
  - `deploy_to_vnpt.sh` (Sửa đổi)
- **Lệnh git commit**: `git commit -m "refactor(infra): rewrite deploy script to use sshpass without expect"`
